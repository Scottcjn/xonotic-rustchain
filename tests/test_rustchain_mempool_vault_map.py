...
def test_mempool_vault_delivery_files_exist():
    expected = [
        f"{NAME}.map",
        f"{NAME}.bsp",
        f"{NAME}.mapinfo",
...
def test_mempool_vault_mapinfo_and_entities_are_playable():
    ...
    assert "RustChain Mempool Vault" in map_text


def test_mempool_vault_bsp_and_levelshot_have_expected_headers():
